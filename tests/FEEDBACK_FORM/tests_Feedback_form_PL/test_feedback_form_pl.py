...
def test_ff_title_pl(setup_pl: Page) -> None:
    expect(setup_pl.locator("//section[@class='ContactFormSection_section__Afhov']//h2")).to_have_text("Formularz zwrotny")
    setup_pl.get_by_role("heading", name="Formularz zwrotny").click()


def test_ff_active_send_btn_pl(setup_pl: Page) -> None:
...
def test_ff_hover_send_btn_pl(setup_pl: Page) -> None:
    setup_pl.get_by_role("button", name="Wyślać").hover()


def test_ff_default_send_btn_pl(setup_pl: Page) -> None:
    expect(setup_pl.get_by_role("button", name="Wyślać")).to_be_enabled()


def test_ff_disabled_send_btn_pl(setup_pl: Page) -> None:
...
def test_ff_send_form_pl(setup_pl: Page) -> None:
...
def test_ff_placeholders_pl(setup_pl: Page) -> None:
